...
folder_name= '/vols/cms/hw423/WeekF/Contours'
...
def ContFields(i,j,cm,x,y,res):
    LP= LL_prof_2(cm,i,j,x,y,res,steps=30)
    deltLP = LP-LP.min()
    return deltLP

    
def PlotContG(i,j,x_ia,y_ia,x_xgb,y_xgb,res,deltLP56,deltLP56_xgb,xlim=None,ylim=None):
    ...
    IA_c = plt.contour(x_grid_ia.numpy(), y_grid_ia.numpy(), deltLP56, levels=level,cmap = 'winter',labels = 'IA',linestyles=linestyles,)
    ...
    plt.ylim(ylim)
    # plt.legend()
    plt.savefig(f'{folder_name}/B_Contour_{i}{j}.pdf',bbox_inches='tight')
...
def PlotContours(i,j,res):
    ...
    PlotContG(i,j,x_ia,y_ia,x_xgb,y_xgb,res,deltLP56_ia,deltLP56_xgb)
IA_xy = [[0.7,1.3],[0,2],[0,3],[0,5],[0,3],[0,12]]
XGB_xy = [[0.6,1.4],[-1,2.5],[-1,3],[0,6],[-1,3],[0,30]]
